...
from .routers import notifications, guides, dashboard, users, tags, research, library, resume
...
if env == "production" and (
    ...
    or "*" in [str(origin) for origin in settings.cors_origins]
):
    logging.warning(
        "CORS is set to allow all origins in production! This is a security risk. Set CORS_ORIGINS to trusted domains only."
    )
...
app.include_router(guides.router, prefix="/resources", tags=["Resources"])
app.include_router(dashboard.router, tags=["dashboard"])
app.include_router(users.router, tags=["users"])
app.include_router(tags.router, tags=["tags"])
app.include_router(research.router, tags=["Research Submission & Review"])
app.include_router(library.router, tags=["library"])
app.include_router(resume.router, tags=["Resume Management"])
...
async def http_exception_handler(request: Request, exc: HTTPException):
    logger.warning(
        f"HTTPException: {exc.detail} (status: {exc.status_code}) at {request.url}"
    )
